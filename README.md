...
- Eligible local videos can be loaded into a bounded RAM cache before playback. Files over the limit or systems without enough available memory automatically fall back to normal disk-backed playback. If a cached run terminates in a matching CoreMedia crash, the next launch disables custom memory playback only for that Mac and macOS build; users can retry manually, and an OS update starts a fresh trial. Per-wallpaper limits are selected by installed-memory floor tiers from `Resources/MemoryCachePolicy.json` (8 GB/512 MB, 12 GB/1 GB, 16 GB/2 GB, 24 GB/3 GB, 32 GB/4 GB, 64 GB/6 GB, 128 GB/8 GB) and can be customized before rebuilding.
